...
for x1, y1, x2, y2 in face_locations: 
    ...
    # me.save(f"./temp{r}.png","PNG") # save each face in a png file

    me.save(f"./temp.png","PNG") 
    unknown_picture=face_recognition.load_image_file("./temp.png") 
    unknown_face_encoding=face_recognition.face_encodings(unknown_picture) 
    found=False 
    try: 
        for f, name in encoded_faces: 
            results=face_recognition.compare_faces([f],unknown_face_encoding[0], tolerance=4) 
            if results[0]== True:
                draw.text((y2, x2), name, font=font, fill=(0,255, 0)) 
                found=True
                break 
    ...
    draw.rectangle(((y1, x1), (y2,x2)), fill=None, outline=(0,255,0))
